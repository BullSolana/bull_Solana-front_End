...
export default function FaqItem({ title, content }: FaqItemProps) {
    ...
    return (
        <div className="w-full rounded-2xl px-2 sm:px-10 border border-purple flex flex-col" onClick={() => setOpen(!open)}>
            <div className='py-2 sm:py-10 flex flex-row items-center justify-between'>
                <div className="pl-4 text-white text-base sm:text-5xl font-black">{title}</div>
                <img className='w-6 sm:w-10' src={arrow}></img>
            </div>
            {
                open && (
                    <div className='pb-6 text-sm sm:text-2xl text-white'>
                        {content}
                    </div>
                )
            }
